...
完整可运行代码见 → [`examples/spring-cache-demo/`](../../examples/spring-cache-demo/)
...
**API 接口：**
- `GET /api/cache-demo/products/{id}` - 获取产品（带缓存）
- `GET /api/cache-demo/products/search` - 搜索产品（不缓存）
- `GET /api/cache-demo/stats` - 缓存统计信息
