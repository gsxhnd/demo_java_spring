...
> Spring Cloud 微服务相关内容已拆分至 [Java Spring Cloud](../Spring_Cloud/)。
...
→ [Java Spring 事务管理](./26_Java%20Spring%20事务管理.md)
...
→ [Java Spring MyBatis](./27_Java%20Spring%20MyBatis.md)
...
→ [Java Spring 多数据库](./28_Java%20Spring%20多数据库.md)
...
→ [Java Spring Service 层](./30_Java%20Spring%20Service%20层.md)

### 6.2 配置管理与 Profile

用 Profile 管理多环境配置（dev、test、prod）。用 `@ConfigurationProperties` 把配置绑定到类型安全的 Java 对象。

→ [Java Spring 配置管理](./31_Java%20Spring%20配置管理.md)
...
→ [Java Spring 异步与定时任务](./32_Java%20Spring%20异步与定时任务.md)
...
→ [Java Spring 缓存](./33_Java%20Spring%20缓存.md)
...
→ [Java Spring 文件处理](./34_Java%20Spring%20文件处理.md)
...
→ [Java Spring 单元测试](./35_Java%20Spring%20单元测试.md)

### 7.2 集成测试（@SpringBootTest）

启动完整的 Spring 上下文，测试多个组件协作的行为。比单元测试慢，但更接近真实环境。

→ [Java Spring 集成测试](./36_Java%20Spring%20集成测试.md)
...
→ [Java Spring 测试切片](./37_Java%20Spring%20测试切片.md)
...
→ [Java Spring Security](./38_Java%20Spring%20Security.md)
...
→ [Java Spring 认证与授权](./39_Java%20Spring%20认证与授权.md)

### 8.3 JWT 集成

在前后端分离架构中，JWT（JSON Web Token）是最常用的无状态认证方案。Spring Security + JWT 的集成是一个高频需求。

→ [Java Spring JWT](./40_Java%20Spring%20JWT.md)

### 8.4 Casbin 权限管理

**是什么：** Casbin 是一个通用的、支持多种访问控制模型（ACL、RBAC、ABAC）的授权库。它把"访问控制模型"和"策略数据"分离，让你用配置文件定义权限规则，策略数据可以存储在文件、数据库、Redis 等任意介质中。

**为什么需要它：** Spring Security 的角色-权限体系在简单场景下够用，但当权限模型变得复杂（角色继承、多租户域隔离、细粒度资源控制、动态策略变更）时，硬编码的注解和 Java Config 会变得难以维护。Casbin 把这些权限规则从代码中抽离出来，让你用声明式的方式管理权限。

**Spring 集成：** `casbin-spring-boot-starter` 提供无缝集成，支持与 Spring Security 配合使用（作为 AuthorizationManager 实现），也可以独立使用。策略存储可选择 JDBC Adapter、Redis Adapter 等。

→ [Java Spring Casbin](./29_Java%20Spring%20Casbin.md)
...
→ [Java Spring WebSocket](./41_Java%20Spring%20WebSocket.md)
...
→ [Java Spring MQTT](./42_Java%20Spring%20MQTT.md)
...
→ [Java Spring Modbus](./43_Java%20Spring%20Modbus.md)
...
→ [Java Spring 通信协议选型](./44_Java%20Spring%20通信协议选型.md)
...
Spring Cloud 微服务相关内容已独立为 [Java Spring Cloud](../Spring_Cloud/) 学习路线。
