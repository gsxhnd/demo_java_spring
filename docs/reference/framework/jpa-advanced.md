...
完整可运行代码见 → [`examples/spring-jpa-advanced-demo/`](../../examples/spring-jpa-advanced-demo/)

**演示功能：**
- Specification 动态查询
- JPA 审计功能（@CreatedDate, @LastModifiedDate, @CreatedBy, @LastModifiedBy）
- 接口投影（Closed Projection）
- DTO 投影
- Flyway 数据库迁移
- 版本控制（@Version 乐观锁）
...
**运行示例：**
```bash
cd examples/spring-jpa-advanced-demo
mvn spring-boot:run
```

**API 接口：**
- `GET /api/posts` - 获取所有文章（分页）
- `GET /api/posts/search` - 动态查询（支持多条件组合）
- `GET /api/posts/summaries` - 接口投影查询
- `GET /api/posts/by-author/{author}` - DTO 投影查询
